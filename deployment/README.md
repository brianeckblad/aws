...
This repo handles **server infrastructure only**. App deployments (SSL, S3 buckets, Secrets Manager, supervisor programs) are managed from each app's own repo.
...
| Playbook | Purpose |
|----------|---------|
| `update-server.yml` | Upgrade OS packages, re-apply config (fail2ban, SSH, sysctl) |
